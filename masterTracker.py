...
def Check_If_Alive(Table_Ip, Lock_Ip_table,port):
    context = zmq.Context()
    socket = context.socket(zmq.SUB)
    for ip in Table_Ip.keys():
        socket.connect("tcp://" + ip + ":" + str(port))   # connect on port with data keeper
    socket.subscribe("")
...
#------------------------------------------------------
#------------------------------------------------------
def n_replicates(Table_Ip, Lock_Ip_table,Table_files,i):
...
def notifyMachineDataTransfer(sourceMachine,copymachine1,Table_Ip,i):
     context = zmq.Context()
     ...
     if(nbFreeportsSource >0):
       ...
       +":"+str(file_tracker_port_address+i))
       socket.send_pyobj(sendingdict,zmq.NOBLOCK) #notify sourcemachine
       socket.connect("tcp://"+str(copymachine1)
       +":"+str(file_tracker_port_address+i))
       socket.send_pyobj(recdict,zmq.NOBLOCK) #notify second machine
...
def response_datakeeper():
 try:
    ...
    response = server.recv_pyobj(zmq.NOBLOCK)
...
#JUST FOR DEBUG ------------
    ...
    for i in range (number_of_tracker_processes):
        portnb=str(5559+i)
        ID = multiprocessing.Process(target= MasterTracker ,
        args=(Table_Ip, Table_files, Lock_Ip_table, Lock_file_table, portnb) )
        ID_List.append(ID)
        ID.start()
        #ID.join()
    multiprocessing.Process(target=Operation_confirmation,
    args=(Table_Ip, Table_files, Lock_Ip_table, 
    Lock_file_table,file_tracker_port_address)).start()
    #for i in range (number_of_tracker_processes):
        #ID_List[i].join()
    for i in range (number_of_tracker_processes):
        portnb=str(check_alive_port_address +i)
        ID1=multiprocessing.Process(target=Check_If_Alive,
        args=(Table_Ip,  Lock_Ip_table,portnb,))
        ID_List2.append(ID1)
        ID1.start()
        ID2=multiprocessing.Process(target=n_replicates,
        args=(Table_Ip,Lock_Ip_table,Table_files,i))
        ID_List.append(ID2)
        ID2.start()
    for i in range (number_of_tracker_processes):
        ID_List2[i].join()
